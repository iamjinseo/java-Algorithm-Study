...
public class B15662_톱니바퀴2 {
	static int[][] gears; // 톱니바퀴들
	static int numOfGears;
	public static void main(String[] args) throws NumberFormatException, IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		numOfGears = Integer.parseInt(br.readLine());
		gears = new int[numOfGears][8];
		...
		for (int i = 0; i < gears.length; i++) {
			if (gears[i][0] == 1) {
				sum++; // 총 K번 회전시킨 이후에 12시방향이 S극인 톱니바퀴의 개수를 출력한다.
			}
		}
	...
	static void turn(int gear, int dir) {
		int[] turnDirs = new int[numOfGears]; // 각 톱니바퀴 회전방향
